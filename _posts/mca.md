...
会发现添加监听后的`personModel`的类从 `BYPersonModel` 变成了`NSKVONotifying_BYPersonModel`，也就是`NSKVONotifying_+类名`
的形式。
...
首先可以发现`setName:`方法的指针指向变了，从`[BYPersonModel setName:]`指向了 `Foundation `的 `_NSSetObjectValueAndNotify`
的C语言方法

`_NSSetObjectValueAndNotify`内部做了什么呢？通过越狱手机可以获取`Foundation`框架，使用`Hopper`
来解析源码生成的是汇编语言，看看汇编源码会发现`_NSSetObjectValueAndNotify`内部注释有提示说调用`didChangeValueForKey`
